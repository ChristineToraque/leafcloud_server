...
def calculate_continuous_targets(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    print('\n📈 Calculating continuous time-based depletion targets (V8)...')
...
def evaluate_classification(y_true_idx, clf_preds, export_dir):
    ...
    plt.title("Confusion Matrix - Nutrient Classification (V8)", fontsize=14, pad=15)
    plt.ylabel("Actual Label", fontsize=12)
    plt.xlabel("Predicted Label", fontsize=12)
    plt.tight_layout()

    os.makedirs(export_dir, exist_ok=True)
    save_path = os.path.join(export_dir, "confusion_matrix_v8.png")
